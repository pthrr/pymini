                         ...
                         setup_pointer settings)        //!< pointer to machine settings      
{
  ...
  if(block->radius_flag || block->theta_flag) {
      ...
      CHKS((!_readers[(int)'x'] || !_readers[(int)'y']), EMC_I18N("Cannot use polar coordinate on a machine lacking X or Y axes"));
      CHKS(((settings->plane != CANON_PLANE_XY)), EMC_I18N("Cannot use polar coordinate except in G17 plane"));
      CHKS(((block->x_flag)), EMC_I18N("Cannot specify both polar coordinate and X word"));
      CHKS(((block->y_flag)), EMC_I18N("Cannot specify both polar coordinate and Y word"));
  }
  ...
  if (mode1 != -1) {
    if (mode1 == G_80) {
      CHKS(((polar_flag || axis_flag) && (!mode_zero_covets_axes)),
          NCE_CANNOT_USE_AXIS_VALUES_WITH_G80);
      CHKS((polar_flag && mode0 == G_92), EMC_I18N("Polar coordinates can only be used for motion"));
  ...
  CHKS((polar_flag && block->motion_to_be == -1), EMC_I18N("Polar coordinates can only be used for motion"));
  return INTERP_OK;
}
