...
int Interp::execute(const char *command)
{
   ...
   {    // copy parameter settings from parameter buffer into parameter table
      ...
      CHP(store_named_param(_setup.named_parameters[n], _setup.named_parameter_values[n]));

      // free the string
      logDebug("freeing param[%d]:|%s|:0x%x\n", n, _setup.named_parameters[n], _setup.named_parameters[n]);
   ...
   {    /* line not blank */
      ...
      if ((status != INTERP_OK) && (status != INTERP_EXECUTE_FINISH) && (status != INTERP_EXIT))
         ERP(status);
   }
...
int Interp::init()
{
   ...
   if (pars[5210])
   {
      ...
      SET_ORIGIN_OFFSETS(USER_TO_PROGRAM_LEN(pars[k + 1] + pars[5211]),
                         ...
                         USER_TO_PROGRAM_LEN(pars[k + 8] + pars[5218]), USER_TO_PROGRAM_LEN(pars[k + 9] + pars[5219]));
   }
   else
   {
      ...
      SET_ORIGIN_OFFSETS(USER_TO_PROGRAM_LEN(pars[k + 1]),
                         ...
                         USER_TO_PROGRAM_LEN(pars[k + 7]), USER_TO_PROGRAM_LEN(pars[k + 8]), USER_TO_PROGRAM_LEN(pars[k + 9]));
   }
...
int Interp::open(const char *filename)  //!< string: the name of the input NC-program file
{
   ...
   for (index = -1; index == -1;)
   {    /* skip blank lines */
      CHKS((fgets(line, LINELEN, _setup.file_pointer) == NULL), NCE_FILE_ENDED_WITH_NO_PERCENT_SIGN);
      length = strlen(line);
      if (length == (LINELEN - 1))
...
int Interp::read(const char *command)   //!< may be NULL or a string to read
{
   ...
   {
      CHKS((GET_EXTERNAL_QUEUE_EMPTY() == 0), NCE_QUEUE_IS_NOT_EMPTY_AFTER_PROBING);
      set_probe_data(&_setup);
      _setup.probe_flag = OFF;
   }
   if (_setup.toolchange_flag == ON)
   {
      CHKS((GET_EXTERNAL_QUEUE_EMPTY() == 0), EMC_I18N("Queue is not empty after tool change"));
   ...
   {
      CHKS((GET_EXTERNAL_QUEUE_EMPTY() == 0), NCE_QUEUE_IS_NOT_EMPTY_AFTER_INPUT);
      if (_setup.input_digital == ON)
      { // we are checking for a digital input
         _setup.parameters[5399] = GET_EXTERNAL_DIGITAL_INPUT(_setup.input_index, (_setup.parameters[5399] != 0.0));
      }
      else
      { // checking for analog input
         _setup.parameters[5399] = GET_EXTERNAL_ANALOG_INPUT(_setup.input_index, _setup.parameters[5399]);
      }
      _setup.input_flag = OFF;
   }
   CHKN(((command == NULL) && (_setup.file_pointer == NULL)), INTERP_FILE_NOT_OPEN);
   ...
   if (command)
      logDebug("Interp::read:[cmd]:|%s|\n", command);
   else
      logDebug("Interp::read:|%s|\n", _setup.linetext);
...
int Interp::restore_parameters(const char *filename)    //!< name of parameter file to read  
{
   ...
   while (feof(infile) == 0)
   {
      ...
      {
         CHKS(((variable <= 0) || (variable >= RS274NGC_MAX_PARAMETERS)), NCE_PARAMETER_NUMBER_OUT_OF_RANGE);
...
int Interp::ini_load(const char *filename)
{
   ...
   snprintf(RS274NGC_PARAMETER_FILE, sizeof(RS274NGC_PARAMETER_FILE), "%s/%s", USER_HOME_DIR, 
      ini_get(filename, "RS274NGC", "PARAMETER_FILE", inistring, sizeof(inistring), (char *)"stepper.var", 1));
