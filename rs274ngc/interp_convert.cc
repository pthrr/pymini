...
#endif
    ERS("G95 set units per revolution mode is not supported. Use G33 or G76.\n");
                            ...
                            setup_pointer settings)     //!< pointer to machine settings             
{
  ...
  if ((settings->cutter_comp_side != OFF) &&    /* ! "== ON" */
    ...
    START_SPEED_FEED_SYNCH(block->line_number, block->k_number, 0);
    STRAIGHT_FEED(block->line_number, end_x, end_y, end_z, AA_end, BB_end, CC_end, u_end, v_end, w_end);
    STOP_SPEED_FEED_SYNCH(block->line_number);
    settings->current_x = end_x;
    settings->current_y = end_y;
    settings->current_z = end_z;
  } else if (move == G_33_1) {
...
threading_pass(setup_pointer settings, block_pointer block,
    ...
    STRAIGHT_TRAVERSE(block->line_number, boring?
		      ...
		      safe_x - depth + end_depth,
		      start_y, start_z - zoff, AABBCC); //back (no cutting)
    if(taper_dist && entry_taper) {
	DISABLE_FEED_OVERRIDE();
	START_SPEED_FEED_SYNCH(block->line_number, taper_pitch, 0);
	STRAIGHT_FEED(block->line_number, boring? 
		      safe_x + depth - full_threadheight: 
		      safe_x - depth + full_threadheight,
		      start_y, start_z - zoff, AABBCC); //in (cut)
	STRAIGHT_FEED(block->line_number, boring? safe_x + depth: safe_x - depth, //angled in (cut)
		      start_y, start_z - zoff - taper_dist, AABBCC);
	START_SPEED_FEED_SYNCH(block->line_number, pitch, 0);
    } else {
	STRAIGHT_TRAVERSE(block->line_number, boring? safe_x + depth: safe_x - depth, 
			  start_y, start_z - zoff, AABBCC); //in (no cutting)
	DISABLE_FEED_OVERRIDE();
	START_SPEED_FEED_SYNCH(block->line_number, pitch, 0);
    }
        
    if(taper_dist && exit_taper) {
	STRAIGHT_FEED(block->line_number, boring? safe_x + depth: safe_x - depth,  //over (cut)
		      start_y, target_z - zoff + taper_dist, AABBCC);
	START_SPEED_FEED_SYNCH(block->line_number, taper_pitch, 0);
	STRAIGHT_FEED(block->line_number, boring? 
		      safe_x + depth - full_threadheight: 
		      safe_x - depth + full_threadheight, 
		      start_y, target_z - zoff, AABBCC); //angled out (cut)
    } else {
	STRAIGHT_FEED(block->line_number, boring? safe_x + depth: safe_x - depth, 
		      start_y, target_z - zoff, AABBCC); //over (cut)
    }
    STOP_SPEED_FEED_SYNCH(block->line_number);
    STRAIGHT_TRAVERSE(block->line_number, boring? 
		      safe_x + depth - end_depth:
		      safe_x - depth + end_depth,
		      start_y, target_z - zoff, AABBCC); //out (no cutting)
    ENABLE_FEED_OVERRIDE();
}
...
int Interp::convert_threading_cycle(block_pointer block, 
    ...
    double pitch = block->p_number;    // distance per revolution
    ...
    settings->current_y = end_y;
    settings->current_z = end_z;
    DBG("Interp::convert_threading_cycle() passes=%d spring_passes=%d", pass, spring_cuts);
#undef AABBC
    return INTERP_OK;
}
