...
/* Tenative code to ignore "%" charactor at begining of a line. DES 4/30/2017 */
int Interp::read_percent(char *line,     //!< string: line of RS274 code being processed   
                           int *counter,   //!< pointer to a counter for position on the line
                           block_pointer block,    //!< pointer to a block being filled from the line
                           double *parameters)     //!< array of system parameters                   
{
  ...
  CHKS((line[*counter] != '%'), NCE_BUG_FUNCTION_SHOULD_NOT_HAVE_BEEN_CALLED);
  CHKS((*counter != 0), EMC_I18N("Bad character '%c' used"), line[*counter]);
  (*counter) = strlen(line);
  return INTERP_OK;
}
