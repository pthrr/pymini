...
void START_SPEED_FEED_SYNCH(int line_number, double feed_per_revolution, bool velocity_mode)
{
   /* Enable synchronized start using index pulse on INPUT0. */
   flush_segments();
   emc_start_speed_feed_synch_msg_t sync_start = { {EMC_START_SPEED_FEED_SYNCH} };
   sync_start.feed_per_revolution = feed_per_revolution;
   sync_start.velocity_mode = velocity_mode;
   interp_list.set_line_number(line_number);
   interp_list.append((emc_command_msg_t *) & sync_start);
   synched = 1;
}

void STOP_SPEED_FEED_SYNCH(int line_number)
{
   flush_segments();
   emc_msg_t sync_stop = { EMC_STOP_SPEED_FEED_SYNCH };
   interp_list.set_line_number(line_number);
   interp_list.append((emc_command_msg_t *) & sync_stop);
   synched = 0;
}
...
/* Tool Functions
 *
 * There are two copies of the tool table at runtime - struct emc_session (emc.h) and 
 * struct setup_struct (interp_internal.h). At startup the *.tbl file is loaded
 * into emc_session then the interpreter reads it from the emc_session. After startup
 * the interpreter uses it's table internally and any changes at runtime are reflected
 * in the emc_session table here. Any runtime changes are not saved in the *.tbl file.
 */

/* This is called with distances in external (machine) units (G10) */
void SET_TOOL_TABLE_ENTRY(int pocket, int toolno, EmcPose offset, double diameter, double frontangle, double backangle, int orientation)
{
   ...
   flush_segments();
   ps->toolTable[toolno].toolno = toolno;
   ps->toolTable[toolno].offset = offset;
   ps->toolTable[toolno].diameter = diameter;
   ps->toolTable[toolno].frontangle = frontangle;
   ps->toolTable[toolno].backangle = backangle;
   ps->toolTable[toolno].orientation = orientation;
}

/*
  EMC interpreter has no tool length offset. To implement it, we save it here,
  and apply it when necessary (G43)
  */
