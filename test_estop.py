#!/usr/bin/python
# test_estop.py - A command line test script. Tests pymini EStop command.
#
...
#
# IMPORTANT!!! Best to run this script offline, CNC controller off. Each EStop 
# will move the origin to a random location.
#
...
#=======================================================================
def usage():
   print("test_estop %s, rt-stepper dongle estop test" % (Version.release))
   print("(c) 2013-2017 Copyright Eckler Software")
   print("David Suffield, dsuffiel@ecklersoft.com")
   print("usage: test_estop [-i inifile] -f gcode.nc [-s serial_number] [-p loop_count]")
...
snum = ""
ini = "rtstepper.ini"
gfile = "offset_test_modified_h.nc"
loopcnt = 1
try:
...
while (loopcnt):
    ...
    # Run gcode with random estop.
    cfile = "%s.estop" % (gfile)
    event = MechEvent(guiq, cfile)  # kickoff event thread
    print("%s Creating %s output with random EStop" % (tmstamp(), gfile))
    mech = Mech(dog, gfile)  # kickoff mech thread
    time.sleep(random.uniform(0.1, tm))
    dog.estop()  # start asynchronized stop
    mech.close()
    time.sleep(0.3)  # wait for event_thread to catch up
    last_estop_line = event.last_line()
    if (last_estop_line == 0):
        last_estop_pos = last_pos   # estop occured before any move was started
    else:
        last_estop_pos = event.last_posistion()
    event.close()
    print("%s EStop after line number %d/%d" % (tmstamp(), last_estop_line, last_line))

    # Clean up after estop.
    dog.estop_reset()
    dog.home()

    # Verify estop file is subset of expected.
            ...
            while (act != ''):
                ...
                if (exp_line != act_line):
                    ...
                    if (bump):
                        ...
                        continue
                    print("Error estop line not equal exp=%s act=%s" % (exp, act))
                    sys.exit(1)
